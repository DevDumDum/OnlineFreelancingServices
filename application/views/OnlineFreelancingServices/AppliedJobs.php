...
$posts = $this->db
    ...
    ->where("post.applicants LIKE '$id' OR post.applicants LIKE '%,$id' OR post.applicants LIKE '$id,%' OR post.applicants LIKE '%,$id,%'")
    // ->where("post.accepted NOT LIKE '$id' and post.accepted NOT LIKE '%,$id' and post.accepted NOT LIKE '$id,%' and post.accepted NOT LIKE '%,$id,%'")
    // ->group_end()
      ...
      <table class="table table-dark table-hover center">
        ...
        <tr>
          ...
          <td style="text-align: right">
            ...
            <?php if ($applicant->id != $id) {
                                    continue;
                                } ?>
            <?php if ($post->accepted) : ?>
              <?php if ($post->email_client == 0): ?>
            <button onclick="sendEmail('<?=$post->ID?>', '<?=$post->email?>')" class="editbtn1 editbtn1-responsiveness" style="cursor: pointer;">Email Client</button>
            <?php endif ?>
        ...
        <tr>
          <td colspan="3">
            <div style="color: red">No Applied Jobs
            </div>
          </td>
        </tr>
        <?php endif ?>
